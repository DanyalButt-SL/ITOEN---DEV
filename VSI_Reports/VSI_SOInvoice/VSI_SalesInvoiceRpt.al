report 50115 "VSI_SalesInvoice"
{
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            //Add Begin By Javeria Akber Against Sales Invoice Report- ITEON
            column(RegionUS; "RegionUS")//north
            {

            }
            //Add End By Javeria Akber Against Sales Invoice Report- ITEON
            ...
            dataitem("Sales Shipment Header"; "Sales Shipment Header")
            {
                ...
                // DataItemLink = "PostingDescription" = FIELD(PostingDescription);
                ...
                column(Shipment_Method_Code_SS; "Shipment Method Code")
                {

                }
                column(Shipment_Date_SS; "Shipment Date")
                {

                }
                column(External_Document_No_SS; "External Document No.")
                {

                }
                column(Order_Date_SS; "Order Date")
                {

                }
                column(Location_Code_SS; "Location Code")
                {

                }


                dataitem("Sales Header"; "Sales Header Archive")
                {
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                //Add Begin By Javeria Akber Against ITEON SalesInvoice Report On 8/15/2020
                column(QTYBO; QTYBO)
                {

                }
                column(GDTHSTPerc; GDTHSTPerc)
                {

                }
                column(GDTHST; GDTHST)
                {
                ...
                column(GDT_HST; GDT_HST)
                {

                }
                //Add End By Javeria Akber Against ITEON SalesInvoice Report On 8/15/2020
                //Add Begin By Javeria Akber Against ITEON SalesInvoice Report On 8/15/2020
                dataitem(SalesLine; "Sales Line Archive")
                {
                    DataItemLink = "Document No." = FIELD("Order No."), "Line No." = FIELD("Line No.");
                    ...
                    column(Quantity_Lbl; FieldCaption("Quantity"))
                    {
                    }
                    ...
                    column(Quantity_Shipped_Lbl; FieldCaption("Quantity Shipped"))
                    {
                    }
                ...
                begin
                    ...
                    //Add Begin By Javeria Against ITEON- Sales Invoice Report On 8/17/2020

                    reservationEntry.Reset();
                    reservationEntry.SetRange("Item No.", Line."No.");
                    reservationEntry.SetRange("Source ID", Line."Order No.");
                    reservationEntry.SetRange("Source Ref. No.", Line."Line No.");

                    if reservationEntry.FindSet then begin
                        repeat
                            if lotId <> '' then begin
                                lotId += '-' + reservationEntry."Lot No." + '(' + '(' + FORMAT(reservationEntry."Quantity (Base)", 7, '<Integer Thousand,3><Decimals,3>') + ')';



                            end else begin
                                lotId += reservationEntry."Lot No." + '(' + FORMAT(reservationEntry."Quantity (Base)", 7, '<Integer Thousand,3><Decimals,1>') + ')';
                                ;
                            end;
                    ...
                    //Add End By Javeria Against ITEON- Sales Invoice Report On 8/17/2020
            ...
            begin
                ...
                //Comment Begin By Javeria Akber fro error on copying from ootb report
                ...
                if Customer.FindSet then begin
                    ...
                    if Customer."Country/Region Code" = 'CA' then begin
                        RegionUS := true;
                    end else begin
                        RegionUS := false;
